...
> &
  Partial<Pick<Prisma.ContractUncheckedCreateInput, 'id' | 'number' | 'createdAt' | 'updatedAt'>>;
...
export class ContractRepository {
  ...
  async findMany(params?: ContractFindManyParams): Promise<Contract[]> {
    ...
    const args: Prisma.ContractFindManyArgs = {
      ...rest,
      where: {
        ...where,
  ...
  async findUnique(
    ...
    // Явная аннотация типа нужна, чтобы разорвать рекурсивный вывод типов
    // (query-extension $extends + спред локального типа → TS2321 excessive stack depth).
    const args: Prisma.ContractFindFirstArgs = {
      where: { id, deletedAt: null },
      include,
    };
    return prisma.contract.findFirst(args);
  }
  ...
  async convertFromDeal(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (!deal.contactId) {
        throw new Error(
          `Cannot convert deal ${dealId} to contract: deal has no contact`
        );
      }

      // Create contract from deal (within transaction).
      // Explicit type annotation breaks the recursive type inference
      // (query-extension $extends + spread → TS2322 Exact<...> mismatch).
